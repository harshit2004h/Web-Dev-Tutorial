...
async function getPlaylist(folder) {
  ...
  for (let i = 0; i < s.length; i++) {
    const element = s[i];
    if (element.href.includes("/Playlists/")) Playlist.push(element.href);
  }
  return Playlist;
}
...
async function getArtist(folder) {
  ...
  for (let i = 0; i < s.length; i++) {
    const element = s[i];
    if (element.href.includes("/Artists/")) Artist.push(element.href);
  }
  return Artist;
}
...
async function main() {
  ...
  let Artists = await getArtist(Folders[3]);
  console.log(Artists);
  let Playlists = await getPlaylist(Folders[4]);
  console.log(Playlists);

  let ArtistUL = document.querySelector(".PopularArtists");
  let j = 0;
  for (const Artist of Artists) {
    let div = document.createElement("div");
    div.setAttribute("class", `PopularCards Art${j}`);
    div.innerHTML =
      div.innerHTML +
      `<div class="PopCard">
                <img id="Artclick" src="Popular Artists/${
                  Artist.split("/Artists/")[1]
                }.jpeg" alt="" />
                <div class="green">
                  <i class="fa-solid fa-play" style="color: #000000"></i>
                </div>
                <h4>${Artist.split("/Artists/")[1].replaceAll("_", " ")}</h4>
                <p>Artist</p>
              </div`;
    ArtistUL.append(div);
    j++;
  ...
  let PlaylistUL = document.querySelector(".SpotifyPlaylist");
  let k = 0;
  for (const Playlist of Playlists) {
    let div = document.createElement("div");
    div.setAttribute("class", `PlayListCards Play${k}`);
    div.innerHTML =
      div.innerHTML +
      `<div class="PlayListCards">
              <div class="PlayCard">
                <img src="PlayLists/${
                  Playlist.split("/Playlists/")[1]
                }.jpeg" alt="" />
                <div class="green">
                  <i class="fa-solid fa-play" style="color: #000000"></i>
                </div>
                <h4 class="${Playlist.split("/Playlists/")[1].replaceAll(
                  "_",
                  " "
                )}">${Playlist.split("/Playlists/")[1].replaceAll(
        "_",
        " "
      )}</h4>
                <p>Playlist ${k + 1}</p>
    ...
    PlaylistUL.append(div);
    k++;
  }

  const Art = document.querySelectorAll(".PopularCards");
  Art.forEach((e) => {
    e.addEventListener("click", () => {
      console.log(e.getElementsByTagName("h4").innerHTML);
    });
  });

  const Play = document.querySelectorAll(".PlayListCards");
  Play.forEach((e) => {
    e.addEventListener("click", () => {
      console.log(e.getElementsByTagName("h4"));
    });
  });

  songs = await getSongs(`http://127.0.0.1:5500/Songs/Artists/Pritam`);
  console.log(songs);
